...
def load_and_derive(csv_path):
    ...
    for col in [
        ...
        "candidate_pool_size", "selected_local_idx", "selected_local_prob", "selected_local_action_rank",
...
def plot_metrics(row_df, ep_df, plots_dir):
    ...
    curves = [
        ...
        ("gate_pass_num", zh("每个 episode 的 gate pass 比例", "Per-episode gate-pass ratio"), zh("比例", "ratio"), (0, 1),
         zh("对 video_gated_fusion / video_all_fusion 有意义。越高表示越常允许高视频权重。", "Meaningful for video_gated_fusion / video_all_fusion. Higher means high video weight is allowed more often.")),
        ("candidate_pool_ratio", zh("每个 episode 的 cluster 候选池比例", "Per-episode cluster candidate-pool ratio"), zh("比例", "ratio"), (0, 1),
         zh("对 video_cluster_fusion / video_all_fusion 有意义。越低表示 cluster gate 剪掉的候选越多。", "Meaningful for video_cluster_fusion / video_all_fusion. Lower means the cluster gate prunes more candidates.")),
    ]
...
def summarize_run(run_name, cfg, success_df, bins_df, row_df, ep_df):
    ...
    if "gate_pass_num" in row_df.columns:
        ...
        if pd.notna(gate):
            lines.append(f"- mean gate_pass={gate:.3f}; meaningful for video_gated_fusion and video_all_fusion.")
    if "candidate_pool_ratio" in row_df.columns:
        pool_ratio = safe_mean(row_df["candidate_pool_ratio"])
        if pd.notna(pool_ratio):
            lines.append(f"- mean candidate_pool_ratio={pool_ratio:.3f}; meaningful for video_cluster_fusion and video_all_fusion.")
